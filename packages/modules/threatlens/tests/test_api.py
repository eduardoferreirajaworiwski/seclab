...
def build_client(db_session):
    app = FastAPI()
    app.include_router(router, prefix="/api/v1/threatlens")
...
def test_create_and_fetch_digest_via_api(db_session):
    ...
    create_response = client.post("/api/v1/threatlens/digests", json={"offline_mode": True})
    assert create_response.status_code == 200
    payload = create_response.json()
    assert payload["articles"]
    assert payload["summary"]["headline"]

    fetch_response = client.get(f"/api/v1/threatlens/digests/{payload['digest_id']}")
...
def test_list_recent_digests_via_api(db_session):
    client = build_client(db_session)
    client.post("/api/v1/threatlens/digests", json={"offline_mode": True})

    list_response = client.get("/api/v1/threatlens/digests?limit=5")
